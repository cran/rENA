...
---
`r accum = enaset$enadata`
`r ub = enaset$enadata$function.params$units.by`
`r u = enaset$enadata$units`
`r cvb = enaset$enadata$function.params$conversations.by`
`r cv = enaset$enadata$conversations`
`r cvEx = as.vector(as.matrix((accum$raw[1,cvb,with=F]))[1,])`
`r codes = enaset$enadata$codes`
`r wb = enaset$enadata$function.params$window.size.back`
`r wgt = enaset$enadata$function.params$weight.by`
`r normed = identical(enaset$function.params$norm.by, rENA:::sphere_norm_c)`
...
`r if(methods == T && !is.null(comparison)) paste0("To test for differences we applied a ", if(comparison == "parametric") "two-sample t-test assuming unequal variance" else "Mann-Whitney test", " to the location of points in the projected ENA space for units in ", comparison.groups[1], " and ", comparison.groups[2], ".")`
...
`r if(methods == T && tool=="webENA") paste0("Marquart, C. L., Hinojosa, C., Swiecki, Z., & Shaffer, D. W. (2018). Epistemic Network Analysis (Version ", tool.version, ") [Software]. Available from http://app.epistemicnetwork.org")``r if(methods == T && tool=="rENA") paste0("Marquart, C. L., Swiecki, Z., Collier, W., Eagan, B., Woodward, R., & Shaffer, D. W. (2018). rENA: Epistemic Network Analysis (Version ", tool.version, "). Retrieved from https://cran.r-project.org/web/packages/rENA/index.html")`

`r if(theory == T) paste0("Ruis, A. R. (In press). “Trois Empreintes d’un Même Cachet”: Toward a Historical Definition of Nutrition. Viral Networks.")`

`r if(theory == T) paste0("Ruis, A. R., Rosser, A. A., Quandt-Walle, C., Nathwani, J. N., Shaffer, D. W., & Pugh, C. M. (in press). The hands and head of a surgeon: Modeling operative competency with multimodal epistemic network analysis. American Journal of Surgery.")`
